...
if (!$training) {
    header('Location: my_trainings.php?msg=not_found');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_training'])) {
    ...
    $stmt = $pdo->prepare("UPDATE trainings SET employee_name=?, training_type=?, division=?, department=?, section=?, unit=?, title_of_activity=?, date_from=?, date_to=?, venue=?, hospital_order=?, date_filed=?, ob_ot=?, ptr_deadline=?, ptr_file=?, remarks=? WHERE id=? AND user_id=?");
    
    if ($stmt->execute([$employee_name, $training_type, $division, $department, $section, $unit, $title_of_activity, $date_from, $date_to, $venue, $hospital_order, $date_filed, $ob_ot, $ptr_deadline, $ptr_file, $remarks, $id, $_SESSION['user_id']])) {
...
<div class="form-row">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <?php if ($training['ptr_file']): ?>
                                    <br><a href="<?= $training['ptr_file'] ?>" target="_blank"><i class="fas fa-file-pdf"></i> Current File</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <a href="my_trainings.php" class="btn-submit" style="background: #666; text-decoration: none;"><i class="fas fa-times"></i> Cancel</a>
                </form>
            </div>
        </div>
    </div>
